...
const generateTable = async (
  ...
  const sqlCreateTable = generateSql(
    config,
    schema,
    epsg,
    dropSchema,
...
const generateSql = (config, schema, epsg, dropSchema, dropTable) => {
  ...
  for (let idTable in config.tableConfig) {
    ...
    if (confTable.geomField && confTable.geomField.name) {
      ...
      tableCol.push(
        `${confTable.geomField.name} geometry(${geometryType}, ${epsg}) NOT NULL`
      );
    }
    ...
    if (confTable.geomField && confTable.geomField.name) {
      sqlBlockTable = [
        ...sqlBlockTable,
        `CREATE INDEX IF NOT EXISTS spidx_${
          confTable.geomField.name
        }_${table.replace(".", "_")} ON ${table} USING gist (${
          confTable.geomField.name
        });`,
      ];
    }
    ...
    if (confTable.pgFkCONSTRAINT) {
      fkBlocks = [
        ...fkBlocks,
        ...confTable.pgFkCONSTRAINT.map((t) =>
          t.replace(/\$schema\$/g, schema)
        ),
      ];
    }
...
const pushGeojsonToPostgis = async (
  configs,
  data,
  schema,
  epsg,
  ...
  for (const tableName in data.geojsons) {
    ...
    for (const feature of features) {
      ...
      let geom = `ST_SetSRID( ST_GeomFromGeoJSON('${JSON.stringify(
        feature.geometry
      )}') ,${epsg})`;
      prop["geom"] = geom;
    ...
    const insert =
      pgp.helpers.insert(dataFeatures, cs) + "ON CONFLICT DO NOTHING";
...
module.exports = {
  ...
  pushGeojsonToPostgis: pushGeojsonToPostgis,
  dbConnect,
  generateTable,
};
